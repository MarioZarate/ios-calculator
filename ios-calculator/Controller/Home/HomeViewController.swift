...
final class HomeViewController: UIViewController {
    ...
    @IBAction func numberDecimalAction(_ sender: UIButton) {
        let currentTmp = auxTotalFormatter.string(from: NSNumber(value: temp))!
        if !operating && currentTmp.count >= kMaxLength {
            return
        }
    ...
    @IBAction func numberAction(_ sender: UIButton) {
        ...
        var currentTmp = auxTotalFormatter.string(from: NSNumber(value: temp))!
        if !operating && currentTmp.count >= kMaxLength {
            return
        }
        
        currentTmp = auxFormatter.string(from: NSNumber(value: temp))!
        ...
        // Decimal Operation
        if decimal {
            currentTmp = "\(currentTmp)\(kDecimalSeparator)"
            decimal = false
        }
        let number = sender.tag
        temp = Double(currentTmp + String(number))!
        resultLabel.text = printFormatter.string(from: NSNumber(value: temp))
        selectVisualOperation()
        sender.shine()
    }
